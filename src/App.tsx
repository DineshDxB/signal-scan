        ...
        <div style={{ marginLeft: 'auto', padding: '0 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {ft.signalCount > 0 && (
            <div style={{ fontSize: 9, fontFamily: 'monospace', padding: '4px 10px', borderRadius: 6, background: '#00aaff11', border: '1px solid #00aaff33', color: '#00aaff' }}>
              {ft.signalCount} signals
            </div>
          )}
          <button onClick={() => setShowSettings(!showSettings)} style={{
            fontSize: 10, fontFamily: 'monospace', padding: '6px 12px', borderRadius: 6,
            cursor: 'pointer', background: showSettings ? '#ffffff11' : 'transparent',
            border: '1px solid #ffffff08', color: '#555'
          }}>⚙ SETTINGS</button>
        </div>
      </div>
        ...
        <div style={{ background: '#0a0a1a', border: '1px solid #ffffff08', borderRadius: '0 0 12px 12px', padding: 16, margin: '0 16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <button onClick={() => handleSaveSettings(settings)} style={{
                width: '100%', padding: 8, fontFamily: 'monospace', fontSize: 11,
          ...
          <div style={{ marginTop: 8, fontSize: 9, fontFamily: 'monospace', color: '#333' }}>
            Wallet ${settings.wallet} · Risk {settings.riskPercent}% = ${(settings.wallet * settings.riskPercent / 100).toFixed(0)} max loss/trade
          </div>
        </div>
      )}
